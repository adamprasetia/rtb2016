		...
		if (!$this->upload->do_upload()){
			...
			if($active_sheet->getCell('A1')->getValue()=='No'){
				...
				while(trim($active_sheet->getCell('A'.$i)->getValue())<>''){
					$data[] = array(
						'fullname'=>trim($active_sheet->getCell('B'.$i)->getValue())
						,'nickname'=>trim($active_sheet->getCell('C'.$i)->getValue())
						,'dob'=>trim($active_sheet->getCell('F'.$i)->getValue()).'-'.trim($active_sheet->getCell('E'.$i)->getValue()).'-'.trim($active_sheet->getCell('D'.$i)->getValue())
						,'tlp'=>trim($active_sheet->getCell('G'.$i)->getValue())
						,'email'=>trim($active_sheet->getCell('H'.$i)->getValue())
